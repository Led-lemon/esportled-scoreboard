...
por SO (`.dmg`/`.exe`/`.AppImage`). Ver `ndi-app/README.md` (build y NDI).
```bash
...
| `ndi-app/` | App de escritorio (Electron): sirve el front, abre el Control y emite las salidas por **NDI con alfa**. Empaqueta por SO con electron-builder. Ver `ndi-app/README.md`. |
